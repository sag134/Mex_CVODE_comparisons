function [err, timepoints, species_out, observables_out ] = MichMent( timepoints, species_init, parameters, suppress_plot )
%MICHMENT Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'MichMent' and then (optionally) plots the observable trajectories,
...
%   source in file MichMent_cvode.c must be compiled (see that file for details).
%   MICHMENT returns an error value, a vector of timepoints,
...
try 
    % run simulation
    [err, species_out, observables_out] = MichMent_cvode( timepoints, species_init, parameters );
catch
...
if ( ~suppress_plot )
    ...
    % define plot labels
    observable_labels = { 'E', 'SU', 'SP', 'ES' };

    % construct figure
    plot(timepoints,observables_out);
    title('MichMent observables','fontSize',14,'Interpreter','none');
...
function [species_init] = initialize_species( params )
    ...
    species_init(1) = 0.01*(602*params(1));
    species_init(2) = 1.0*(602*params(1));
    species_init(3) = 0;
    species_init(4) = 0;
